...
function [VS_T_new, VS_dQ, VS_dT, count_itr, total_rays] = equilibriumRad(N_rays,VS_T,VS_T_fixed,voxel_space,varargin)
    ...
    while count_final_level <= final_level_itrs
        ...
        if N_bands == 1
            ...
            VS_T_new(VS_PM) = (VS_Q_emit_new(VS_PM)./(prod(vx_scale)*4.*VS_nn(VS_PM).^2*sigma.*VS_PM_kappa(VS_PM))).^(1/4);
            VS_T_new(VS_surf_areas>0) = (VS_Q_emit_new(VS_surf_areas>0)./(VS_nn(VS_surf_areas>0).^2.*sigma.*VS_surf_areas(VS_surf_areas>0).*VS_opaq_eps(VS_surf_areas>0))).^(1/4);        
