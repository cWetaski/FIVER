...
D_TES = 67/1000; % m;
D_TES = 0.1; % m
N_grid_D = 30; % Number of gridcells across the diameter
r_vx_pipe = 1;
L = 1.5*D_TES; % m
...
spectral_band_edges = [spectral_band_edges;100];
N_rays = 5e5;
ns = 2;
...
%% Charging parameters
charge_T = 1500; % K: Temperature of charging radiation
charge_D = 12/1000; % 12 mm diameter rod
charge_L_frac = 0.5; % charging rod fraction of TES length (assume it is centered in the TES)
eps_charge = 1; % Blackbody
...
r_pipe = r_vx_pipe*vx_scale;
D_pipe = r_pipe*2;

VS_T_init = zeros(N_grid_D,N_grid_D,N_grid_L);
size_VS = size(VS_T_init);

charge_power = charge_L_frac*L*pi/4*charge_D*eps_charge*5.67e-8*charge_T^4; % W
...
%% Line flux definition
line_p1 = [size_VS(1)/2,size_VS(2)/2,size_VS(3)/2*(1-charge_L_frac)];
line_p2 = [size_VS(1)/2,size_VS(2)/2,size_VS(3)/2*(1+charge_L_frac)];
...
line_flux_ray_fun = @(N_rays) [ray_pos_generator(N_rays),ray_dir_generator(N_rays)];
...
for nn = 1:N_band
    ...
    charge_flux_power_band = charge_L_frac*L*pi/4*charge_D*eps_charge*spectralBandPower(band_l,band_u,charge_T,1)
    internal_flux = InternalFlux(voxel_space,charge_flux_power_band,line_flux_ray_fun);
...
%% Figure
T_2D_init_E = VS_T_init(:,:,floor(size_VS(3)/2)+1);
for nn = 1:N_pipe
    T_2D_init_E(VS_pipe_2D{nn}) = nan;
end
f = figure;
zrange = [(T_TES_i-50-273.15),(T_TES_i-273.15+200)];
...
for tt = 1:N_time_steps
    ...
    while q_rel_diff > 0.1 && count < 3
        ...
        for nn = 1:N_band
            ...
            if band_l >= 0.26 && band_u < 5
                PM_kappa = getMeanBK7coeffViskanta(band_l,band_u); % 1/m
                n_refrac = getMeanRefractiveIndex(band_l,band_u);
                if PM_kappa*vx_scale > 10 % Optical thickness of 1 voxel exceeds 10
                    nom_emissive_power = spectralBandPower(band_l,band_u,VS_T_new,n_refrac); % Nominal emissive power
                    k_Rosseland = k_Rosseland + nom_emissive_power*16/3/PM_kappa; % Add to Rosseland conductivity
                end
            end
        end
        ...
        M = M_trans-M_diff+M_BC+M_pipe+M_source_term; %
        RHS = RHS_trans+RHS_BC+RHS_pipe+RHS_source_term; %+RHS_pipe;
    ...
    T_old = T_new;
    % Update plot
    T_2D_new_E = VS_T_new(:,:,floor(size_VS(3)/2)+1); % Get a 2D cross section
    for nn = 1:N_pipe
