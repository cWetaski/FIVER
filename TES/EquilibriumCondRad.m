...
%                                                           solution.
%   t_step_factor (scalar double):                          Used to scale time stepping, set >1 if convergence is too
%                                                           slow and set <1 if solution is diverging.
...
%   residuals_steady (3D double):       Residual of final temperature field from steady state heat transfer problem    
    ...
    default_t_step_scaling = 1; % Scaling how far forward to time step each iteration (set <1 if solution is diverging)
    ...
    addParameter(parser,'TimeStepScaling',default_t_step_scaling)
    ...
    t_step_scaling = parser.Results.TimeStepScaling;
    ...
    if voxel_spaces{1}.thermal_conductivity == 0
        idx = find(cellfun(@(s) strcmp('TimeStepScaling',s),varargin));
        if ~isempty(idx) % Must delete this argument to pass it to Equilibrium rad
            varargin(idx)=[];
            varargin(idx)=[];
        end
    ...
    N_param = max_PM_kappa/mean(VS_alpha(:))/(4*sigma*max(VS_T(:))^3); % Estimate conduction-to-radiation factor (see Modest pp 725)

    dt = t_step_scaling*1./max_PM_kappa^2*N_param/(2*mean(VS_alpha(:))); % [s]: Time step as N_param gets smaller, time step also gets smaller since more risk of diverging when radiation dominates, time step decreases with increasing PM_kappa since solution is exploding otherwise
    ...
    %% Initialize counters
    count_level = 0; % Counter which resets each time N_rays level is incremented
    ...
    while count_final_level < final_level_itrs
        ...
        VS_dQ = MC_RadiativeHeatFlows(N_rays(cur_level),VS_T_prev,voxel_spaces,"SpectralBands",spectral_bands,"OutputMode",output_mode); % (3D Double) [W/vx^3]: Radiative flux divergence

        VS_source_term = VS_dQ./prod(vx_scale); % [W]: convert divergence of heat flux to a source term
        ...
        [M_trans, RHS_trans] = transientTerm(T_prev, dt); % Get transient term based on old temperature field. Don't quite understand how this works but it works
        ...
        if count_level > N_prev % The first N_prev iterations at a level, we just store the temperature fields
            ...
            R_sq_ratio = R_sq_N_dT/R_sq_2_dT; % Ratio of square residuals.
        ...
        % T_nondim = mean(VS_T_prev,[2 3])/max(VS_T_prev(:));
